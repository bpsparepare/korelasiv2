...
class _RecapUsersScreenState extends State<RecapUsersScreen> {
  ...
  Future<void> _fetchAvailableMonthsAndInitialData() async {
    ...
    try {
      final response =
      await supabase.from('korelasi_data_pesanan').select('tanggal_pesanan');
      ...
      _availableMonths = uniqueMonths.toList()..sort();
      ...
      if (mounted) setState(() => _errorMessage = 'Gagal memuat daftar bulan: $e');
    } finally {
    ...
    try {
      ...
      for (var orderItem in (response as List)) {
        ...
        userRecapMap.update(
          ...
          ifAbsent: () =>
              UserRecap(id: userId, nama: userName, totalSpending: subtotal),
        );
      }

      _currentRecap = userRecapMap.values.toList()
  ...
  Future<void> _exportCurrentRecapToXlsx() async {
    ...
    try {
      ...
      excel.sheets.keys.where((s) => s != 'Sheet1').toList().forEach(excel.delete);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Rekapitulasi Pengguna',
        ...
        children: [
                  ...
                  children: [
                    Expanded(
                      child: DropdownButtonFormField<String>(
                        ...
                        items: ['Semua Waktu', ..._availableMonths]
                            .map((String month) {
          ...
          Expanded(
            child: RefreshIndicator(
              onRefresh: () =>
                  _fetchUserRecapsForMonth(_selectedMonth),
